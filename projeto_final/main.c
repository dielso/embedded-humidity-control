...
void main(void) {
    ...
    rgbInit(); // inicializa os periféricos
    lcdInit();
    adcInit();
    ...
    umidade = 0xa5; // lê pela primeira vez o serial
    while(umidade == 0xa5){
    ...
    for (;;) {
        ...
        case 1:
            TRISD = 0x00; 
            if (umidade<(limiar-10) || umidade>(limiar+10)){ // se a umidade estiver fora do nivel aceitável [...]
                
                // o controle fica aqui! um bitflip (x = x ^ 0) faria o trabalho
                // ou seja: se estiver ligado, desliga; caso contrário, liga
